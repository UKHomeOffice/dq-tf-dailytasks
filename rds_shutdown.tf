...
data "archive_file" "rds_shutdownzip" {
  type   =  "zip"
  source_file = "${local.path_module}/lambda/code"
  output_path = "${local.path_module}/lambda/package/rds_shutdownzip"
}
...
resource "aws_lambda_function" "rds-shutdown-function" {
    ...
    filename = "${path.module}/lambda/package/rds_shutdownzip"
    memory_size = 128
    timeout = "10"
